...
install_xiahai_xiaoming() {
    ...
    if [[ -f /opt/xiahai-xiaoming/chrome-sandbox ]]; then
        # Only this package-verified Chromium sandbox receives the privileged
        # mode required by Electron.  Never apply this permission recursively.
...
# Ming Store is deployed with the desktop assets.  This module deliberately
# installs no Spark/APM/ACE runtime; upgrade cleanup happens in 07_finalize
# without autoremove so applications previously installed by Spark remain.
