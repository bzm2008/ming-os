...
def apply_firewall_atomic(candidate, runner=run_command):
    ...
    table_existed = snapshot_rc == 0
    if snapshot_rc != 0:
        list_rc, tables, list_error = runner(["nft", "list", "tables"])
        if list_rc != 0:
            return {"ok": False, "rolled_back": False,
                    "error": list_error or snapshot_error or "unable to inspect firewall"}
        if re.search(r"(?m)^\s*table\s+inet\s+ming_filter\s*$", tables):
            return {"ok": False, "rolled_back": False,
                    "error": snapshot_error or "unable to snapshot firewall"}
        snapshot = ""
    transaction = candidate
    if table_existed:
        transaction = "destroy table inet ming_filter\n" + candidate.lstrip()
    check_rc, _output, check_error = runner(
        ["nft", "-c", "-f", "-"], transaction)
    if check_rc != 0:
        return {"ok": False, "rolled_back": False,
                "error": check_error or "firewall validation failed"}
    apply_rc, _output, apply_error = runner(["nft", "-f", "-"], transaction)
    if apply_rc == 0:
        return {"ok": True, "rolled_back": False, "error": "",
                "snapshot": snapshot, "snapshot_existed": table_existed}
    if not table_existed:
        return {"ok": False, "rolled_back": True,
                "error": apply_error or "firewall commit failed", "rollback_error": ""}
...
def rollback_configuration(old, kind, rules_path, runner, persist_rules=False,
    ...
    if kind == "security-updates":
        ...
        restore_rules = nft_snapshot if nft_snapshot is not None else firewall_rules(old)
