...
def validate_user(user, lookup=None):
...
def caller_may_change(user, environ=None, uid_lookup=None):
    ...
    if uid_lookup is None:
        return False
    try:
        caller_uid = int(pkexec_uid)
        account = uid_lookup(caller_uid)
        return caller_uid >= 1000 and account.pw_uid == caller_uid and account.pw_name == user
    except (KeyError, TypeError, ValueError):
        return False
...
def retire_skipped_marker(user, marker_path=None, expected_uid=None):
    try:
        ...
        # A first-time password setup has no skipped marker to retire.
        return True
    except OSError:
        return False
